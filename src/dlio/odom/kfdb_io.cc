...
namespace dlio::kfdb
{
  ...
  bool save(const std::string &path,
            const std::vector<dlio::AppearanceEntry> &entries,
            float sc_max_range,
  ...
  bool load(const std::string &path,
            std::vector<dlio::AppearanceEntry> &database,
            float &sc_max_range,
            Eigen::Quaternionf &gravity_q)
  {
    ...
    for (uint32_t i = 0; i < num_entries; i++)
    {
      ...
      entry.descriptor.sc_descriptor.resize(dlio::sc::SC_NR, dlio::sc::SC_NS);
      ifs.read(reinterpret_cast<char *>(entry.descriptor.sc_descriptor.data()),
               dlio::sc::SC_NR * dlio::sc::SC_NS * sizeof(float));

      entry.descriptor.ring_key.resize(dlio::sc::SC_NR);
      ...
      entry.descriptor.sector_key = dlio::sc::computeSectorKey(entry.descriptor.sc_descriptor);
