...
void c_lio::OdomNode::computeMetrics(pcl::PointCloud<PointType>::ConstPtr scan)
{
  this->computeSpaciousness(scan);
  this->computeDensity();
}

void c_lio::OdomNode::computeSpaciousness(pcl::PointCloud<PointType>::ConstPtr scan)
{
...
void c_lio::OdomNode::computeDensity()
{
  ...
  else if (this->engine_.needsCovariances())
  {
    density = this->engine_.getSourceDensity();
  }
  else
  {
    density = this->gicp_max_corr_dist_; // NDT/robust_icp has no density metric
  }
...
void c_lio::OdomNode::computeConvexHull()
{
...
void c_lio::OdomNode::computeConcaveHull()
{
...
void c_lio::OdomNode::updateKeyframes()
{
...
void c_lio::OdomNode::pushSubmapIndices(std::vector<float> dists, int k, std::vector<int> frames)
{
...
void c_lio::OdomNode::buildSubmap(State vehicle_state)
{
    ...
    else
    {
      ...
      for (auto k : this->submap_kf_idx_curr)
      {
        ...
        *submap_cloud_ += *this->keyframes[k].second;
        ...
        if (this->engine_.needsCovariances() && this->keyframe_normals[k])
        {
...
bool c_lio::OdomNode::buildPriorMapRoiSubmap(const State &vehicle_state)
{
...
void c_lio::OdomNode::buildKeyframesAndSubmap(State vehicle_state)
{
  if (this->deep_debug_)
  ...
  for (int i = this->num_processed_keyframes; i < this->keyframes.size(); i++)
  {
    if (this->deep_debug_)
      RCLCPP_INFO(this->get_logger(), "[DEEP] buildKF&Submap: processing kf[%d], cloud=%zu pts, normals=%zu",
                  ...
                  (this->engine_.needsCovariances() && i < (int)this->keyframe_normals.size() && this->keyframe_normals[i]) ? this->keyframe_normals[i]->size() : 0);
    ...
    if (this->engine_.needsCovariances() && this->keyframe_normals[i])
    {
      if (this->deep_debug_)
    ...
    std::thread(&c_lio::OdomNode::publishKeyframe, this, kf_data, kf_ts, kf_cloud).detach();
...
void c_lio::OdomNode::pruneLocalizationKeyframes()
{
...
void c_lio::OdomNode::pauseSubmapBuildIfNeeded()
{
