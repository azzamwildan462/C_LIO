...
void c_lio::OdomNode::loadPriorMap()
{
...
// SC computation functions moved to c_lio/scan_context.h

void c_lio::OdomNode::buildScanContextDatabase()
{
...
bool c_lio::OdomNode::runRelocalization(pcl::PointCloud<PointType>::ConstPtr scan)
{
  ...
  c_lio::AppearanceDescriptor query_desc = this->appearance_.computeDescriptor(sc_scan);
...
std::string c_lio::OdomNode::getKfdbPath() const
{
  return c_lio::kfdb::getKfdbPath(this->map_path_, this->use_corrected_);
}

void c_lio::OdomNode::computeAndStoreKeyframeSC()
{
...
bool c_lio::OdomNode::saveKeyframeDatabase()
{
  ...
  std::vector<c_lio::AppearanceEntry> entries_snap;
  float max_range;
  ...
  bool ok = c_lio::kfdb::save(kfdb_path, entries_snap, max_range, gravity_q);
  if (ok)
...
bool c_lio::OdomNode::saveCorrectedKeyframeDatabase()
{
  ...
  std::lock_guard<std::mutex> file_lock(this->kfdb_file_mutex_);
  bool ok = c_lio::kfdb::saveCorrected(this->map_path_, entries_snap,
...
bool c_lio::OdomNode::loadKeyframeDatabase()
{
  std::string kfdb_path = this->getKfdbPath();
  if (kfdb_path.empty())
    return false;
  ...
  bool ok = c_lio::kfdb::load(kfdb_path, this->sc_database_,
