...
extern std::atomic<c_lio::OdomNode *> g_odom_node;

void c_lio::OdomNode::publishPose()
{
...
void c_lio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud,
                                   ...
                                   Eigen::Matrix4f T_cloud)
{
  ...
  // Otherwise always publish it — T_map_odom_ defaults to Identity() and is only
  // mutated by continuousLocalize()/submap localization, so this is safe even
  // when both are disabled (e.g. continuous_localize:false for CPU savings) —
  // without this, disabling those correction features also silently broke the
  // map->odom hop and left the TF tree disconnected from map.
...
void c_lio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud,
                                   pcl::PointCloud<PointType>::ConstPtr raw_deskewed_cloud,
                                   Eigen::Matrix4f T_cloud)
{
                                      ...
                                      pcl::PointCloud<PointType>::ConstPtr local_cloud)
{
  ...
  auto kf_msg = std::make_unique<c_lio::msg::KeyframeStamped>();
...
void c_lio::OdomNode::publishMode()
{
  if (!this->mode_pub_)
    return;
...
void c_lio::OdomNode::publishPriorMap()
{
...
void c_lio::OdomNode::applyModeTransition(const std::string &new_mode)
{
...
void c_lio::OdomNode::srvSetMode(
    std::shared_ptr<c_lio::srv::SetMode::Request> req,
    std::shared_ptr<c_lio::srv::SetMode::Response> res)
{
  ...
  if (req->mode == "mapping" && !this->map_path_.empty())
  {
    ...
    if (this->kfdb_entries_.empty())
    {
      ...
      std::vector<c_lio::AppearanceEntry> loaded;
      if (c_lio::kfdb::load(kfdb_path, loaded, file_max_range, file_gravity_q))
      {
...
void c_lio::OdomNode::reloadPriorMapForRelocalization()
{
...
void c_lio::OdomNode::srvRelocalize(
    std::shared_ptr<c_lio::srv::Relocalize::Request> /*req*/,
...
void c_lio::OdomNode::callbackInitialPose(
...
void c_lio::OdomNode::srvSetPose(
    std::shared_ptr<c_lio::srv::SetPose::Request> req,
...
void c_lio::OdomNode::srvGetState(
    std::shared_ptr<c_lio::srv::GetState::Request> /*req*/,
...
bool c_lio::OdomNode::callSavePCD()
{
  ...
  auto request = std::make_shared<c_lio::srv::SavePCD::Request>();
  ...
  this->save_pcd_client_->async_send_request(request,
                                             [logger](rclcpp::Client<c_lio::srv::SavePCD>::SharedFuture future)
                                             {
...
bool c_lio::OdomNode::callSaveCorrectedPCD()
{
...
void c_lio::OdomNode::clearAllMapData()
{
  std::lock_guard<std::mutex> state_lock(this->state_mtx_);
...
void c_lio::OdomNode::srvNewMap(
    std::shared_ptr<c_lio::srv::NewMap::Request> /*req*/,
...
void c_lio::OdomNode::srvNewMapWZero(
    std::shared_ptr<c_lio::srv::NewMapWZero::Request> /*req*/,
...
void c_lio::OdomNode::debug()
{
  ...
  if (this->fusion_method_ == c_lio::FusionMethod::KF)
    fusion_str = "kf";
  else if (this->fusion_method_ == c_lio::FusionMethod::EKF)
    fusion_str = "ekf";

  // Motion model string
  const char *mm_str = "none";
  if (this->motion_model_type_ == c_lio::MotionModelType::ACKERMANN)
    mm_str = "ackermann";
  else if (this->motion_model_type_ == c_lio::MotionModelType::DIFF_DRIVE)
    mm_str = "diff_drive";
  else if (this->motion_model_type_ == c_lio::MotionModelType::HOLONOMIC)
    mm_str = "holonomic";
  ...
  std::cout << std::endl
            << "+-------------------------------------------------------------------+" << std::endl;
  std::cout << "|               Compact LiDAR-Inertial Odometry v" << this->version_ << "               |" << std::endl;
  std::cout << "+-------------------------------------------------------------------+" << std::endl;
...
void c_lio::OdomNode::publishEarthToMapTF()
{
...
void c_lio::OdomNode::continuousLocalize()
{
  ...
  std::vector<c_lio::AppearanceEntry> sc_snap;
  ...
  // Rotate scan to gravity-aligned frame for descriptor computation (with ground removal)
  auto sc_scan = this->appearance_.prepareGravityAlignedScan(
      scan_body, this->kfdb_gravity_q_, this->extrinsics.baselink2lidar.R);

  auto sc_desc = this->appearance_.computeDescriptor(sc_scan);
  ...
  if (!gicp_ok && this->enable_global_correction_.load())
  {
    ...
    for (int ci = 0; ci < n_stage2_candidates; ci++)
    {
      ...
      Eigen::Vector3f cand_pos = sc_snap[kf_idx].position;
      float sc_yaw = this->appearance_.shiftToYaw(top_candidates[ci].sc_shift);
...
bool c_lio::OdomNode::verifyLoopWithG2O(
    int loop_kf_idx,
...
void c_lio::OdomNode::publishOccupancyGrid()
{
  if (!this->occupancy_grid_gen_ || !this->c_lio_initialized)
    return;
