...
void c_lio::OdomNode::resetGtsamNavState()
{
...
void c_lio::OdomNode::getScanFromROS(const sensor_msgs::msg::PointCloud2::SharedPtr &pc)
{
  ...
  this->sensor = c_lio::SensorType::UNKNOWN;
  for (auto &field : pc->fields)
  {
    if (field.name == "t")
    {
      this->sensor = c_lio::SensorType::OUSTER;
      break;
    }
    else if (field.name == "time")
    {
      this->sensor = c_lio::SensorType::VELODYNE;
      break;
    }
    else if (field.name == "timestamp")
    {
      ...
      if (ts0 > 1e14)
      {
        this->sensor = c_lio::SensorType::LIVOX; // nanoseconds since epoch
      }
      else if (ts0 > 1e6)
      {
        this->sensor = c_lio::SensorType::HESAI; // absolute seconds since epoch
      }
      else
      {
        this->sensor = c_lio::SensorType::ROBOSENSE; // relative offset in seconds from scan start
      }
      break;
    }
    else if (field.name == "time_stamp")
    {
      ...
      this->sensor = c_lio::SensorType::ROBOSENSE;
      break;
    }
  }

  if (this->sensor == c_lio::SensorType::UNKNOWN)
  {
    this->deskew_ = false;
  }
...
void c_lio::OdomNode::preprocessPoints()
{

  // Deskew the original c_lio-type scan
  if (this->deskew_)
  {
  ...
  if (this->vf_use_)
  {
    if (this->gpu_preprocess_)
    {
      ...
      c_lio::cuda::DeviceBuffer<float4> d_in, d_out;
      d_in.upload(h_in.data(), N);
      d_out.allocate(N);

      size_t n_out = c_lio::cuda::voxelGridFilter(
...
void c_lio::OdomNode::deskewPointcloud()
{
  ...
  if (this->sensor == c_lio::SensorType::OUSTER)
  {
  ...
  else if (this->sensor == c_lio::SensorType::VELODYNE)
  {
  ...
  else if (this->sensor == c_lio::SensorType::HESAI)
  {
  ...
  else if (this->sensor == c_lio::SensorType::ROBOSENSE)
  {
  ...
  else if (this->sensor == c_lio::SensorType::LIVOX)
  {
    point_time_cmp = [](const PointType &p1, const PointType &p2)
    { return p1.timestamp < p2.timestamp; };
    point_time_neq = [](boost::range::index_value<PointType &, long> p1,
  ...
  if (this->gpu_preprocess_)
  {
    ...
    c_lio::cuda::DeviceBuffer<float4> d_points;
    c_lio::cuda::DeviceBuffer<float> d_transforms;
    c_lio::cuda::DeviceBuffer<int> d_frame_indices;
    ...
    c_lio::cuda::deskewPointCloud(d_points.data(), d_points.data(), N,
...
void c_lio::OdomNode::initializeC_LIO()
{
  ...
  this->c_lio_initialized = true;
  std::cout << std::endl
            << " C_LIO initialized!" << std::endl;
}

void c_lio::OdomNode::callbackPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc)
{
  ...
  // C_LIO Initialization procedures (IMU calib, gravity align)
  if (!this->c_lio_initialized)
  {
    this->initializeC_LIO();
  }

  // Set initial pose for prior map (non-relocalize mode)
  if (this->use_prior_map_ && !this->relocalize_ && !this->prior_map_pose_set_ && this->c_lio_initialized)
  {
  ...
  // Convert incoming scan into C_LIO format
  ...
  this->metrics_thread = std::thread(&c_lio::OdomNode::computeMetrics, this, scan_for_metrics);
  ...
  if (this->keyframes.size() == 0)
  {
    ...
    this->submap_future =
        std::async(std::launch::async, &c_lio::OdomNode::buildKeyframesAndSubmap, this, this->state);
  ...
  if (this->motion_model_type_ != c_lio::MotionModelType::NONE)
  {
    ...
    {
    case c_lio::MotionModelType::ACKERMANN:
      ...
      break;
    case c_lio::MotionModelType::DIFF_DRIVE:
      ...
      break;
    case c_lio::MotionModelType::HOLONOMIC:
...
void c_lio::OdomNode::callbackExternalOdom(const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr msg)
{
...
void c_lio::OdomNode::callbackImu(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
{
...
sensor_msgs::msg::Imu::SharedPtr c_lio::OdomNode::transformImu(const sensor_msgs::msg::Imu::SharedPtr &imu_raw)
{
