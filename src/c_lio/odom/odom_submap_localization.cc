...
void c_lio::OdomNode::initSubmapLocalization()
{
...
void c_lio::OdomNode::submapLocalizeTick()
{
  // Prerequisites
  if (!this->c_lio_initialized || !this->relocalized_)
    return;
...
void c_lio::OdomNode::submapLocalizeStage1(
    pcl::PointCloud<PointType>::ConstPtr scan_body,
    const Eigen::Matrix4f &T_odom_body)
{
  ...
  c_lio::AppearanceDescriptor query_app_desc;
  ...
  if (sc_filter_enabled)
  {
    ...
    query_app_desc = this->appearance_.computeDescriptor(sc_scan);
  }
  ...
  for (size_t si = 0; si < this->submap_entries_.size(); si++)
  {
    ...
    if (sc_filter_enabled)
    {
      ...
      for (int ki : sm.kf_indices)
      {
        ...
        auto [dist, shift] = this->appearance_.compareDescriptors(
            query_app_desc, this->sc_database_[ki].descriptor);
        if (dist < min_sc_dist)
          min_sc_dist = dist;
      }
    ...
    {
      auto reg_result = this->loc_registration_.align(scan_body, sm.cloud, init_guess);
...
void c_lio::OdomNode::submapLocalizeStage2(
    pcl::PointCloud<PointType>::ConstPtr scan_body,
    const Eigen::Matrix4f &T_odom_body)
{
...
int c_lio::OdomNode::findClosestKF(const Eigen::Vector3f &pos) const
{
  int closest = 0;
